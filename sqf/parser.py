...
class Parser:
    ...
    def parse_strings_and_comments(self, all_tokens):
        """
        ...
        if mode in ("comment_line", "comment_bulk"):
            ...
            self.raise_parser_error(get_coord(tokens), 'String is not closed')

        return tokens

    def _analyze_simple(self, tokens):
    ...
    def _analyze_array(self, tokens, analyze_tokens, tokens_until):
        result = []
        part = []
        ...
        for token in tokens:
            if token == ParserKeyword(','):
                ...
                if not part:
                    self.raise_parser_error(get_coord(tokens_until), 'Array cannot have an empty element')
    ...
    def _analyze_define(self, tokens):
        ...
        if len(valid_indexes) < 2:
            self.raise_parser_error(get_coord(str(tokens[0])), '#define needs at least one argument')
    ...
    def find_match_if_def(self, all_tokens, i, defines, token):
        found = False
    ...
    def get_ifdef_variable(self, tokens, ifdef_i, coord_until_here):
        variable = None
        eol_i = None
        ...
        self.raise_parser_error(add_coords(coord_until_here, tokens[:ifdef_i]), '#ifdef statement must contain a variable')

    def parse_ifdef_block(self, expression, defines, coord_until_here):
        """
        ...
        variable, eol_i = self.get_ifdef_variable(tokens, ifdef_i, coord_until_here)
    ...
    def parse_block(
        ...
        while i < len(all_tokens):
            ...
            if token in (Preprocessor('#ifdef'), Preprocessor('#ifndef')):
                stop = True
                lvls['ifdef'] += 1
                expression, size = self.parse_block(all_tokens, self._analyze_simple, i + 1, lvls, stop_statement,
                                               defines=defines)
                lvls['ifdef'] -= 1
                if lvls['ifdef'] == 0:
                    assert (isinstance(expression, IfDefStatement))
                    replacing_expression = self.parse_ifdef_block(expression, defines, get_coord(all_tokens[:i - 1]))

                    new_all_tokens = sqf.base_type.get_all_tokens(tokens + replacing_expression)

                    result, _ = self.parse_block(new_all_tokens, analyze_tokens, 0, None, stop_statement,
                                            defines=defines)
                    ...
                    i += size + 1
            # finish ifdef
            elif self.is_finish_ifdef_condition(tokens, lvls) and (
                    self.is_end_statement(token, stop_statement) or
                    self.is_finish_ifdef_parenthesis(token, lvls)
                    ) or lvls['ifdef'] > 1 and token == Preprocessor('#endif'):

                if token != EndOfFile() and token not in CLOSE_PARENTHESIS:
                    tokens.append(token)

                if_def = self.finish_ifdef(tokens, all_tokens, start, statements)
            ...
            # try to match a #defined and get the arguments
                ...
                stop, define_statement, arg_indexes = self.find_match_if_def(all_tokens, i, defines, token)
                ...
                if stop:
                    ...
                    expression, size = self.parse_block(new_all_tokens, analyze_tokens, new_start, lvls,
            ...
            if stop:
                ...
                lvls['[]'] += 1
                expression, size = self.parse_block(all_tokens, analyze_tokens, i + 1, lvls, stop_statement='single', defines=defines)
                lvls['[]'] -= 1
                tokens.append(expression)
                i += size + 1
            elif token == ParserKeyword('('):
                lvls['()'] += 1
                expression, size = self.parse_block(all_tokens, analyze_tokens, i + 1, lvls, stop_statement, defines=defines)
                lvls['()'] -= 1
                tokens.append(expression)
                i += size + 1
            elif token == ParserKeyword('{'):
                ...
                if lvls['[]'] == 0:
                    self.raise_parenthesis_error(all_tokens[:i], 'Trying to close right parenthesis without them opened.')

                if statements:
                    if isinstance(statements[0], DefineResult):
                        statements[0]._tokens = [Array(self._analyze_array(statements[0]._tokens, analyze_tokens, all_tokens[:i]))]
                        return statements[0], i - start
                    else:
                        self.raise_parser_error(get_coord(all_tokens[:i]), 'A statement %s cannot be in an array' % Statement(statements))

                return Array(self._analyze_array(tokens, analyze_tokens, all_tokens[:i])), i - start
            elif token == ParserKeyword(')'):
                if lvls['()'] == 0:
                    self.raise_parenthesis_error(all_tokens[:i], 'Trying to close parenthesis without opened parenthesis.')
                ...
                if lvls['{}'] == 0:
                    self.raise_parenthesis_error(all_tokens[:i], 'Trying to close brackets without opened brackets.')
            ...
            # end of statement when not in preprocessor states
            elif all(lvls[lvl_type] == 0 for lvl_type in ('#define', '#include')) and self.is_end_statement(token, stop_statement):
                ...
                if tokens[0] == Preprocessor('#define'):
                    define_statement = self._analyze_define(tokens)
        ...
        if self.is_finish_ifdef_condition(tokens, lvls):
            return self.finish_ifdef(tokens, all_tokens, start, statements), i - start
        ...
        for lvl_type in ('[]', '()', '{}', 'ifdef'):
            if lvls[lvl_type] != 0:
                ...
                self.raise_parenthesis_error(all_tokens[:start - 1], message)
    ...
    def parse(self, script):
        tokens = [identify_token(x) for x in self.parse_strings_and_comments(tokenize(script))]

        result = self.parse_block(tokens + [EndOfFile()], self._analyze_tokens)[0]

        result.set_position((1, 1))

        return result

    def raise_parser_error(self, coord: (int, int), message: str):
        raise SQFParserError(self.context.with_position(coord), message)

    def raise_parenthesis_error(self, tokens, message: str):
        raise SQFParenthesisError(self.context.with_position(get_coord(tokens)), message)
