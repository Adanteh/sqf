...
from sqf.exceptions import SQFError, SQFParserError, SQFParenthesisError, SQFWarning
...
class GeneralTestCase(TestCase):
    ...
    def test_warn_not_in_scope(self):
        analyzer = analyze(self.parser.parse('_z;'))
    ...
    def test_assign_wrong(self):
        analyzer = analyze(self.parser.parse('1 = 2;'))
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(1, errors[0].context.col)
    ...
    def test_private_eq1(self):
        analyzer = analyze(self.parser.parse('private _x = 1 < 2; _x'))
        self.assertEqual(analyzer.exceptions, [])
    ...
    def test_private_many(self):
        analyzer = analyze(self.parser.parse('private ["_x", "_y"]; _x; _y'))
    ...
    def test_private_wrong(self):
        ...
        analyzer = analyze(self.parser.parse('private _x; _x'))
        errors = analyzer.exceptions
    ...
    def test_private_wrong_exp(self):
        # private argument must be a string
        analyzer = analyze(self.parser.parse('private {_x};'))
        errors = analyzer.exceptions
    ...
    def test_private_global(self):
        ...
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)
        self.assertIsInstance(errors[0], SQFParserError)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(9, errors[0].context.col)
    ...
    def test_missing_semi_colon(self):
        ...
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 2)
        self.assertEqual(2, errors[0].context.line)
        self.assertEqual(3, errors[0].context.col)
    ...
    def test_statement(self):
        ...
        self.assertEqual(len(errors), 2)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(6, errors[0].context.col)
    ...
    def test_for_missing_do(self):
        ...
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(23, errors[0].context.col)
    ...
    def test_if_missing_then(self):
        ...
        self.assertEqual(len(errors), 2)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(11, errors[0].context.col)
        self.assertEqual(1, errors[1].context.line)
        self.assertEqual(1, errors[1].context.col)
    ...
    def test_error_message_unary(self):
        ...
        self.assertEqual(errors[0].message,
                         'Unary operator "parsenumber" only accepts argument '
    ...
    def test_error_message_binary(self):
        ...
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)
        self.assertTrue(errors[0].message.startswith('Binary operator "+" arguments must be ['))
    ...
    def test_throw(self):
        analyzer = analyze(self.parser.parse('if (true) throw false'))
        self.assertEqual(analyzer.exceptions, [])
...
class Preprocessor(TestCase):
    ...
    def test_ifdef_with_error_pos_in(self):
        ...
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)
        self.assertEqual(4, errors[0].context.line)
        self.assertEqual(1, errors[0].context.col)
    ...
    def test_ifdef_with_error_pos_out(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertEqual(4, errors[0].context.line)
        self.assertEqual(2, errors[0].context.col)
    ...
    def test_same_name(self):
        analyzer = analyze(self.parser.parse('LOG("")'))
        self.assertEqual(analyzer.exceptions, [])
...
class Arrays(TestCase):
    ...
    def test_basic(self):
        ...
        self.assertEqual(len(errors), 2)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(4, errors[0].context.col)
        self.assertEqual(2, errors[1].context.line)
        self.assertEqual(1, errors[1].context.col)
    ...
    def test_error_inside_array(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(7, errors[0].context.col)
    ...
    def test_strange(self):
        ...
        self.assertEqual(len(errors), 2)
        self.assertEqual(3, errors[1].context.line)
        self.assertEqual(8, errors[1].context.col)
    ...
    def test_position_of_array(self):
        ...
        errors = analyzer.exceptions
        self.assertEqual(1, errors[1].context.line)
        self.assertEqual(36, errors[1].context.col)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(23, errors[0].context.col)
...
class Switch(TestCase):
    ...
    def test_case_not_code(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(22, errors[0].context.col)

    def test_incomplete_case(self):
        code = 'switch (x) do {case 1: }'
        analyzer = analyze(self.parser.parse(code))
        self.assertEqual(len(analyzer.exceptions), 1)
        self.assertEqual(1, analyzer.exceptions[0].context.line)
        self.assertEqual(22, analyzer.exceptions[0].context.col)
    ...
    def test_missing_do(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertEqual(1, errors[0].context.line)
        self.assertEqual(12, errors[0].context.col)
...
class NestedCode(TestCase):
    ...
    def test_code_with_private(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertEqual(2, errors[0].context.line)
        self.assertEqual(15, errors[0].context.col)
    ...
    def test_call_within(self):
        analyzer = analyze(self.parser.parse('x = 0; call {call {x = x + 1; call {x = x + 2}}}'))
        self.assertEqual(analyzer.exceptions, [])

    def test_call_within_this(self):
        analyzer = analyze(self.parser.parse('x = {if (call _this) exitWith {call _this}}'))
        self.assertEqual(analyzer.exceptions, [])
...
class Params(TestCase):
    ...
    def test_single(self):
        analyzer = analyze(self.parser.parse('0 params ["_x"]; _x'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertTrue('_x' in analyzer)
    ...
    def test_many(self):
        analyzer = analyze(self.parser.parse('[1,2] params ["_x", "_y"]; _x;_y'))
        self.assertEqual(analyzer.exceptions, [])

    def test_many_call(self):
        analyzer = analyze(self.parser.parse('x = {params ["_x", "_y"]; _x;_y}; [1,2] call x'))
        self.assertEqual(analyzer.exceptions, [])

    def test_with_default(self):
        analyzer = analyze(self.parser.parse('x = [] call {params [["_x", 0]]; _x}'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertEqual(Number(), analyzer['x'])

    def test_with_default_unused(self):
        analyzer = analyze(self.parser.parse('[""] params [["_x", 0]]; _x'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertEqual(String('""'), analyzer['_x'])

    def test_rhs_larger_lhs(self):
        analyzer = analyze(self.parser.parse('[0,1] params ["_x"]; _x'))
        self.assertEqual(len(analyzer.exceptions), 1)

    def test_missing_arg(self):
        analyzer = analyze(self.parser.parse('[0] params ["_x", "_y"]; _x; _y'))
        self.assertEqual(len(analyzer.exceptions), 1)
        self.assertEqual(Nothing(), analyzer['_y'])

    def test_missing_default_arg(self):
        analyzer = analyze(self.parser.parse('[0] params ["_x", ["_y", 2]]; _x + _y'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertEqual(Number(2), analyzer['_y'])

    def test_with_empty_string(self):
        analyzer = analyze(self.parser.parse('params [""]'))
        self.assertEqual(analyzer.exceptions, [])
    ...
    def test_array_wrong(self):
        analyzer = analyze(self.parser.parse('params [["_x"]];'))
        self.assertEqual(len(analyzer.exceptions), 1)

    def test_wrong_array_element(self):
        analyzer = analyze(self.parser.parse('params [1]'))
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)
    ...
    def test_lhs_rhs_single_var(self):
        analyzer = analyze(self.parser.parse('x=2; x params ["_x"]; _x'))
        self.assertEqual(analyzer.exceptions, [])
...
class UndefinedValues(TestCase):
    ...
    def test_number(self):
        ...
        code = "y = 2 + x"
        analyze(self.parser.parse(code), analyzer)
        self.assertEqual(analyzer.exceptions, [])
    ...
    def test_array2(self):
        analyzer = analyze(self.parser.parse('allPlayers select [1,2];'))
        self.assertEqual(analyzer.exceptions, [])

    def test_if1(self):
        # undefined -> do neither and invalidate any assigment
        analyzer = analyze(self.parser.parse('x=2; if (y) then {x=1} else {x=2}'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertEqual(Number(), analyzer['x'])

        # undefined -> test then
        analyzer = analyze(self.parser.parse('if (y) then {!1}'))
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)

        # undefined -> test both
        analyzer = analyze(self.parser.parse('if (y) then {!1} else {!2}'))
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 2)
    ...
    def test_while(self):
        ...
        analyzer = analyze(self.parser.parse('while {x != 0} do {x = 1}'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertEqual(Anything(), analyzer['x'])

        # undefined -> test
        analyzer = analyze(self.parser.parse('while {x != 0} do {! 1}'))
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)

    def test_for(self):
        # undefined -> do not execute
        analyzer = analyze(self.parser.parse('x = 0; for "_i" from 0 to y do {x = x + 1}'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertEqual(Number(), analyzer['x'])

    def test_for1(self):
        # undefined -> test
        analyzer = analyze(self.parser.parse('x = 0; for "_i" from 0 to y do {!1}'))
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)

    def test_forspecs(self):
        # undefined -> do not execute
        analyzer = analyze(self.parser.parse('y = 0; for [{x = 1},{x <= z},{x = x + 1}] do {y = y + 1}'))
        self.assertEqual(analyzer.exceptions, [])
        self.assertEqual(Number(), analyzer['y'])

        # undefined -> test
        analyzer = analyze(self.parser.parse('y = 0; for [{x = 1},{x <= z},{x = x + 1}] do {!1}'))
        errors = analyzer.exceptions
        self.assertEqual(len(errors), 1)
...
class SpecialComment(TestCase):
    ...
    def test_string2_fail(self):
        ...
        self.assertEqual(errors[0].message, 'USES_VARIABLES comment must be `//USES_VARIABLES ["var1",...]`')
